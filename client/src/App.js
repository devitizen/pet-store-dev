...
function Pets() {
    ...
    const getAllUrl = `${serverUrl}/api?act=getall`;
    ...
    function addPet(e) {
        e.preventDefault();
        const url = `${serverUrl}/api?act=add&animal=${animal}&description=${description}&age=${age}&price=${price}`;
        fetch(url)
            .then((res) => res.json())
            .then((result) => {
                fetchPets();
    ...
    function deletePet(id) {
        const url = `${serverUrl}/api?act=delete&id=${id}`;
        fetch(url)
            .then((res) => res.json())
            .then((result) => {
                fetchPets();
    ...
    function updatePet(e) {
        e.preventDefault();
        const url = `${serverUrl}/api?act=update&id=${id}&animal=${animal}&description=${description}&age=${age}&price=${price}`;
        fetch(url)
            .then((res) => res.json())
            .then((result) => {
                fetchPets();
    ...
    function searchPet(searchTerm) {
        const url = `${serverUrl}/api?act=search&term=${searchTerm}`;
        fetch(url)
            .then((res) => res.json())
            .then((result) => {
                setPets(result);
    ...
    if (!isLoaded) {
        ...
        return (
            <Container sx={{ maxWidth: 1024, minWidth: 650 }}>
                <Box sx={{ typography: "h4", textAlign: "center", my: 5 }}>
                    ...
                    <Table aria-label="simple table">
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell sx={{ border: 1, fontSize: 16 }}>
                                    Description
                                </TableCell>
                                <TableCell
                                    sx={{
                ...
                <Dialog open={open} onClose={closeDialog}>
                    <Box
                        component="form"
                        onSubmit={id === 0 ? addPet : updatePet}
                        sx={{ p: "10px" }}
                    >
                        <DialogTitle>
                            {id === 0 ? "Add new pet" : "Edit pet"}
                        </DialogTitle>
                        <DialogContent>
                            <Grid item xs={12}>
                                <TextField
